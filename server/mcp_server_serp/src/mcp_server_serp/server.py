...
from mcp_server_serp.base.config import SERP_CONFIG

logger = logging.getLogger(__name__)

# Initialize FastMCP server
mcp = FastMCP("SERP MCP Server", port=int(os.getenv("PORT", "8000")))


@mcp.tool()
def serp_for_me(query: str = None) -> dict[str, Any]:
    """query a single keyword from some search engineer"""

    try:
        # url = "https://101.47.24.171/v1/queries"
        url = f"https://{SERP_CONFIG.serp_endpoint}/v1/queries"

        headers = {
            "Content-Type": "application/json",
            "X-VE-Source": "google_search",
            # "X-VE-Api-Token": "msm_serp_test@api_token#mcp_server",
            "X-VE-Api-Token": SERP_CONFIG.serp_token,
        }
        data = {
            "query": query,
        ...
        error_message = f"Error when mcp_server_serp: {str(e)}"
